# Krabka vs Strimzi benchmark harness

This end-to-end benchmark compares **Apache Kafka with the Strimzi operator**
against **Krabka with its own operator**. Both stacks run on the same Kubernetes
...
address. The driver is built on Krabka's own `krabka-client-producer` and
`krabka-client-consumer` crates.
...
just -f bench/justfile run krabka small-msg-saturate 1broker-rf1
...
```
bench/results/
  krabka-small-msg-saturate-1broker-rf1.json          # single run, no tag
  kafka-small-msg-saturate-1broker-rf1.json
  krabka-high-partition-saturate-6broker-rf3-run01.json  # 10× averaged run
  krabka-high-partition-saturate-6broker-rf3-run02.json
  ...
...
runs in a cell**. It renders krabka-vs-kafka columns with a `ratio` column. A
...
Krabka is greenfield, and some Kafka features are not available yet. The
benchmark reports this clearly:

- **RF=3 / 3-broker quorum.** The harness tries this only in `cluster` mode. If
  Krabka's broker pods do not all reach Ready, the orchestrator marks the run as
  skipped with `"krabka:rf3-unstable"`. It does not report zeros.
...
`page_cache_approx = working_set − heap − non-heap`. Krabka has no JVM
...
Do not conclude that "Krabka uses 5× less heap than Kafka". The chart
does not show that.
...
- `bench/manifests/krabka/`: Krabka Kafka and KafkaNodePool (1-, 3-, and 6-broker), KafkaTopic.
