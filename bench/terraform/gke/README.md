...
The setup has two three-broker clusters. The Krabka operator manages one, and
...
This configuration matches the live `test-krabka-cluster` in
...
The operator and broker images default to `ghcr.io/krabka-io/krabka-*`. Override
...
docker tag krabka-bench-driver:e2e "$REG/krabka-bench-driver:e2e"
docker push "$REG/krabka-bench-driver:e2e"
export BENCH_DRIVER_IMAGE="$REG/krabka-bench-driver:e2e"

# Pin the operator + broker image tags to the release under test:
export KRABKA_OPERATOR_IMAGE_TAG=v0.3.6
export KRABKA_BROKER_IMAGE_TAG=v0.3.6
```
...
# = install-strimzi + install-krabka + install-prometheus
```

This installs the Krabka operator with Helm, the Strimzi cluster operator that
...
for scenario in small-msg-saturate fan-out mixed-acks large-msg; do
  bench/scripts/run-scenario.sh krabka "$scenario" 3broker-rf3
  bench/scripts/run-scenario.sh kafka  "$scenario" 3broker-rf3
done

# Zero-copy TLS (kTLS) data path — add a 4th `tls` arg:
bench/scripts/run-scenario.sh krabka small-msg-saturate 3broker-rf3 tls
bench/scripts/run-scenario.sh krabka large-msg         3broker-rf3 tls
```

`run-scenario.sh STACK SCENARIO TOPOLOGY [tls]` takes `STACK` as `krabka|kafka`.
...
`bench-report` runs `krabka-bench-report` over `bench/results/*.json`. It renders
...
The defaults here match the live `test-krabka-cluster`. To confirm that the
...
cd bench/terraform/gke
terraform init
terraform import google_container_cluster.bench   PROJECT/us-central1-b/test-krabka-cluster
terraform import google_container_node_pool.default PROJECT/us-central1-b/test-krabka-cluster/default-pool
terraform import google_container_node_pool.beefy   PROJECT/us-central1-b/test-krabka-cluster/beefy-pool
...
- **Resources are identical across stacks.** Both broker CRs request 2-4 vCPU /
  ...
  the CRs under [`bench/manifests/krabka/`](../../manifests/krabka) and
