// GKE provisioner for the Krabka-vs-Strimzi Kubernetes benchmark.
//
// Brings up the GKE Standard cluster the `bench/` harness (Krabka + Strimzi
// operators, Prometheus, the `krabka-bench-driver` Job) runs against. Two
...
//
// This mirrors the live `test-krabka-cluster` in `robot-head/us-central1-b`;
