...
#   listener port, which DIFFERS per stack. krabka reserves 9093 for the KRaft
...
bootstrap_for() {
  ...
  case "$stack" in
    ...
    kafka|strimzi)
      ...
      ;;
    *)
      log "unknown stack '$stack' (want krabka|kafka)"
      return 2
      ;;
  esac
}
...
#     krabka : the shared headless-Service FQDN (a SAN on every broker cert)
...
kafka_kind() {
  local stack="$1"
  case "$stack" in
    krabka)        printf 'kafka.krabka.io/demo' ;;
...
#   (and for Krabka: all BENCH_BROKER_COUNT brokers are listed as ready)
...
wait_kafka_ready() {
  ...
  local kind name
  case "$stack" in
    krabka)        kind="kafka.krabka.io"; name="demo" ;;
  ...
  while :; do
    ...
    if [[ "$status" == "True" ]]; then
      # For Krabka multi-broker: also verify all brokers have joined.
      # The operator sets message like "3/3 brokers ready across 3 pool(s)".
      if [[ "$stack" == "krabka" && "$expected_brokers" -gt 1 ]]; then
...
wait_kafka_topic_ready() {
  ...
  local kind
  case "$stack" in
    krabka)        kind="kafkatopic.krabka.io" ;;
