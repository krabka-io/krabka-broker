...
#
# Env vars:
#   BENCH_DRIVER_IMAGE   image ref for the driver Job (default krabka-bench-driver:e2e)
...
#                        consumer request timeout (default 5s for krabka, 30s for kafka)
...
: "${BENCH_DRIVER_IMAGE:=krabka-bench-driver:e2e}"
...
# named demo-cluster-ca-cert (key ca.crt) for BOTH krabka and Strimzi.
if [[ -n "$BENCH_TLS" ]]; then
