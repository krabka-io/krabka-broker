...
STACK="${1:?stack: krabka|kafka}"

case "$STACK" in
  krabka)
    log "tearing down Krabka cluster 'demo'"
    kubectl delete kafkatopic.krabka.io bench-topic -n "$BENCH_NAMESPACE" --ignore-not-found
    kubectl delete kafka.krabka.io demo -n "$BENCH_NAMESPACE" --ignore-not-found --wait=false
    ...
    kubectl delete kafkanodepool.krabka.io \
      -n "$BENCH_NAMESPACE" \
      -l "krabka.io/cluster=demo" \
      --ignore-not-found 2>/dev/null || true
    # Fallback: also try the old single-pool name in case the label isn't set
    kubectl delete kafkanodepool.krabka.io brokers \
  ...
  case "$STACK" in
    krabka)
      pods=$(kubectl get pods -n "$BENCH_NAMESPACE" \
        -l "app.kubernetes.io/instance=demo,app.kubernetes.io/managed-by=krabka-operator" \
