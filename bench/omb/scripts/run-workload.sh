...
#   run-workload.sh krabka 1-topic-16-partitions-1kb
#
...
stack="${1:-}"; workload="${2:-}"
[[ -n "$stack" && -n "$workload" ]] || die "usage: $0 {kafka|krabka} <workload-name-without-yaml>"

case "$stack" in kafka|krabka) ;; *) die "unknown stack: $stack" ;; esac
