// GCP provisioner for the OMB Krabka-vs-Kafka harness.
//
// Topology (defaults):
//   - 1 VPC + 1 subnet in `var.region`
//   - 3 Kafka broker VMs   (KRaft, combined controller+broker)
//   - 1 Krabka broker VM   (single-node quorum; see README caveat)
...
// Local-SSD-attached VM. Used for both Kafka and Krabka brokers — same
...
resource "google_compute_instance" "krabka_broker" {
  count        = var.num_instances["krabka_broker"]
  name         = "${var.name_prefix}-krabka-${count.index}-${local.suffix}"
  machine_type = var.machine_types["krabka_broker"]
  zone         = var.zone
  tags         = ["${var.name_prefix}-${local.suffix}", "krabka-broker"]
