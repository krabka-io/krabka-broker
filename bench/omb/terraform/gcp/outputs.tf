...
output "krabka_brokers" {
  description = "Krabka brokers (public + private IPs). Capped at 1 today."
  value = [
    for vm in google_compute_instance.krabka_broker : {
      name       = vm.name
      public_ip  = vm.network_interface[0].access_config[0].nat_ip
      private_ip = vm.network_interface[0].network_ip
    }
...
output "krabka_bootstrap_servers" {
  description = "PLAINTEXT bootstrap.servers for the Krabka cluster (private IPs, port 9092)."
  value = join(",", [
    for vm in google_compute_instance.krabka_broker :
    "${vm.network_interface[0].network_ip}:9092"
  ])
