// GCP-side configuration for the OMB Krabka-vs-Kafka harness.
...
variable "machine_types" {
  ...
  default = {
    ...
    krabka_broker = "n2-standard-16"
    client        = "n2-standard-32"
  }
}

variable "num_instances" {
  description = "Per-class VM counts. krabka_broker is hard-capped at 1 today — see bench/omb/README.md."
  type        = map(number)
  default = {
    kafka_broker  = 3
    krabka_broker = 1
  ...
  validation {
    condition     = var.num_instances["krabka_broker"] <= 1
    error_message = "Krabka bare-VM bring-up is single-broker only today. Lower krabka_broker to 1, or use bench/justfile for the K8s/operator path."
  }
}
