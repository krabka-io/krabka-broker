# OpenMessaging Benchmark — Krabka vs Kafka on GCP
...
suite against both **Apache Kafka (KRaft)** and **Krabka**. Both stacks
...
## Why a separate broker count for Krabka and Kafka?

Krabka's bare-VM bring-up supports only **single-broker, RF=1** today.
...
- `krabka_broker_count` defaults to **1**, with a maximum of **1**. The
...
the driver YAML to `1` for both stacks when `krabka_broker_count == 1`,
so the comparison is direct *at RF=1*. Run the KinD harness if you need
RF=3 numbers, because that path uses the Krabka operator.
...
3× `n2-standard-16` for Kafka, 1× for Krabka, 4× `n2-standard-32`
...
# 3. Install Kafka + Krabka + OMB clients (one playbook per stack).
bench/omb/scripts/deploy-stack.sh kafka
bench/omb/scripts/deploy-stack.sh krabka

# 4. Run a workload against one stack, then the other.
bench/omb/scripts/run-workload.sh kafka  1-topic-16-partitions-1kb
bench/omb/scripts/run-workload.sh krabka 1-topic-16-partitions-1kb
...
just -f bench/omb/justfile run krabka 1-topic-16-partitions-1kb
...
per stack on each client VM. Krabka speaks the Kafka wire protocol, so
...
- **No SASL / TLS.** The driver YAML is plaintext, to match the OMB AWS
  reference. Krabka and Kafka both support SASL/SSL. It is simple to add
...
└── ansible/
    ...
    └── templates/
        ...
        ├── kafka.service.j2
        ├── krabka-broker.toml.j2
        ├── krabka-broker.service.j2
