...
with tempfile.TemporaryDirectory() as directory:
    ...
    expected = ["by-dependency", "by-requires", "third-kernel"]
    if actual != expected:
        sys.exit(f"expected {expected}, got {actual}")

    missing = uncovered(workspace, ["by-dependency", "by-requires"])
    if missing != ["third-kernel"]:
        sys.exit(f"expected uncovered third-kernel, got {missing}")
    ...
    if check.returncode == 0 or "third-kernel" not in check.stderr:
        sys.exit(f"expected uncovered third-kernel failure, got {check.stderr!r}")
