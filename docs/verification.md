...
| [`restore_batch_step`](../crates/verified/src/restore.rs) accepts only a non-overlapping archived batch base and computes its exclusive end without overflow, while permitting gaps preserved by Kafka compaction. | The restore verifier's [record-batch walk](../crates/restore/src/verify/log_walk.rs) | [`proof.json`](../verif/krabka_verified_rlib/restore/restore_batch_step/proof.json) | Batch framing and CRC are validated before the host passes the decoded base offset and last-offset delta to the kernel. |
