...
| `krabka_broker_log_cleaner_uncleanable_partitions` | gauge | - | `kafka.log:type=LogCleanerManager,name=uncleanable-partitions-count` | Partitions this broker leads whose most recent compaction attempt failed and which have not compacted since. Republished each sweep, so it falls when a pass succeeds or when this broker stops leading the partition. [Runbook](runbooks/log-cleaner-stalled.md). |
