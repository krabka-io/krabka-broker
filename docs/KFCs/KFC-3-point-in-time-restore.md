...
One thing is still not proved. The restore seeds every `PartitionRecord` at `leader_epoch` 0, so an archive whose batches carry a higher `partition_leader_epoch` restores into a cluster whose metadata epoch is below its own log's, and `OffsetForLeaderEpoch` for the archived epoch answers `UNKNOWN_LEADER_EPOCH` rather than the epoch's end offset. Every fixture here archives at epoch 0, so no test covers the case yet.
