...
//! subcommand writes both ids to `meta.properties.json`.
//!
...
//!
//! Neither id is serialized as a whole value, so neither needs serde. The
//! code writes each with `to_string()` into a hand-built JSON object, and
//! unwraps [`DirectoryId`] to the raw `Uuid` at the `crabka_voters::Voter`
//! boundary.
