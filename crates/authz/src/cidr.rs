...
impl Cidr {
    /// Parses `host` as a CIDR pattern.
    ///
    /// # Errors
    ///
    /// Returns a reason string when `host` is not valid CIDR notation (not
    /// exactly one `/`, an unparseable IP address, or a prefix length past
    /// the address family's maximum); the caller prefixes it with
    /// `Invalid CIDR notation '<host>': ` to match Kafka's `CreateAcls`
    /// message.
    pub fn parse(host: &str) -> Result<Self, String> {
        ...
        let Some(prefix_part) = parts.next() else {
            return Err("expected exactly one '/'".to_owned());
        };
        ...
        if prefix_len > max_prefix_len {
            return Err(format!(
                "prefix length {prefix_len} exceeds the {max_prefix_len}-bit address"
            ));
        }
...
mod tests {
    ...
    fn parse_rejects_malformed_cidrs() {
        let cases: &[(&str, &str)] = &[
            ("not-an-ip/8", "'not-an-ip' is not an IP address"),
            ("10.0.0.0/thirty", "'thirty' is not a valid prefix length"),
            ("10.0.0.0/33", "prefix length 33 exceeds the 32-bit address"),
            (
                "2001:db8::/129",
                "prefix length 129 exceeds the 128-bit address",
            ),
