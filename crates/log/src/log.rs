...
impl Log {
    ...
    fn advance_lso_when_no_open_transaction(&mut self) {
        if self.pending.is_empty() {
            self.lso = self.log_end_offset();
        }
    }
...
/// `RETAIN_EMPTY` rule.
fn control_batch_kind(batch: &RecordBatch) -> Option<ControlBatchKind> {
    if !batch.attributes.is_control_batch() {
        return None;
    }
    let marker_type = batch
...
mod tests {
    ...
    fn barrier_value(group: &str, epoch: i64) -> Bytes {
        let mut buf = Vec::new();
    ...
    fn barrier_marker(group: &str, epoch: i64) -> RecordBatch {
        RecordBatch {
            base_offset: 0,
            last_offset_delta: 0,
    ...
    fn barrier_marker_from_producer(
        group: &str,
        epoch: i64,
        producer_id: i64,
        producer_epoch: i16,
