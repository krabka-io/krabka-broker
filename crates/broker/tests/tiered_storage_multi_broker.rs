...
//! 5. Poll broker 2's log directory, **while broker 1 is still the leader**,
//!    until only the active segment is left. Broker 2 is the follower; it runs
//!    local retention off the shared RLMM, so `local.retention.bytes=1` takes
//!    its copy of every tiered segment before any leadership change.
//! 6. Wait 8s for broker 2's RLMM consumer to read the `CopySegment` events.
//! 7. Shut down broker 1, the partition leader. The surviving quorum of 2 out
//!    of 3 commits a new partition leader record, and broker 2 wins the
//!    election.
//! 8. Consume ALL records from broker 2 at offset 0. They can come only from
//!    the shared remote tier, because broker 2's local log was already evicted
//!    at step 5 and broker 2 never ran the copy task. Broker 2 serves them from
//!    the cached metadata.
//!
//! ## Discriminating property
//!
//! The survivor never ran the copy task for these segments, and its local copy
//! was gone before it was elected -- step 5 waits for exactly that, so the
//! records cannot have come off its own disk. It can serve them only through
//! the shared Local tier and the shared RLMM metadata. With a per-broker
//! in-memory RLMM, the survivor would have no metadata: step 5 would time out
//! and the consume would fail. Do NOT weaken either assertion; the consume must
//! require all records back, and the eviction must be observed before the
//! election rather than after it.
//!
...
//! [`multi_workload`] creates the tiered topic, produces into it and waits for
