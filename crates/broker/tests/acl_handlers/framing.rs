...
//! connection and moves its bytes through these two helpers, so they live
//! apart from the typed request drivers that call them.
...
/// Same shape as `auth_handlers::harness::round_trip`. It encodes a request
/// header (v1 non-flexible, v2 flexible), prepends a 4-byte length prefix,
