...
//!
//! Both helpers are copies of the ones in `client_quotas.rs`. They live in
//! their own module because everything else in the suite is written on top of
//! them: the quota drivers speak through `sasl_plain_authenticate`, and the
//! accept-path tests use `round_trip` on its own to prove that a connection
