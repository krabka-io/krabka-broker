...
/// Where to send a heartbeat addressed to controller `leader`.
///
...
/// forwards a rejected `submit_change` to. Reusing that resolution keeps the
/// heartbeat on the same address the raft transport already reaches the leader
/// on, and avoids three traps that the leader's
/// `ControllerRegistrationRecord` would walk into:
///
...
///   there is no record to read and no heartbeat would ever be sent.
...
pub(crate) async fn run(mut cfg: Config) {
    ...
    loop {
        ...
        let Some((host, port)) = leader_endpoint(&image, &cfg.controller_quorum_voters, leader_id)
        else {
            debug!(
                leader = leader_id.0,
