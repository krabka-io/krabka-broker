...
/// than the whole [`MetadataSource`] keeps the read of the new image on the
/// same channel that woke the loop, so no image can slip past between the
/// wake-up and the read.
