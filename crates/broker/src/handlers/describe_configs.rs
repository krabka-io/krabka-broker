...
mod tests {
    ...
    use krabka_metadata::{
        BrokerConfigRecord, MetadataImage, MetadataRecord, PatternType, TopicConfigRecord,
        TopicFreezeRecord,
    };
    use krabka_protocol::{
        UnknownTaggedFields,
    ...
    fn topic_describe_synthesises_write_freeze_from_the_registry() {
        ...
        for (label, topic, value, config_source) in [
            (
                ...
                super::CONFIG_SOURCE_DYNAMIC_TOPIC,
            ),
            (
    ...
    fn topic_describe_reports_write_freeze_on_a_cluster_with_no_freeze() {
        ...
        assert!(
            describe_topic(&image, "orders", None)
                == topic_result(
                    "orders",
                    vec![freeze_entry("false", super::CONFIG_SOURCE_DEFAULT)],
                )
        );
    }
            ...
            describe_topic(&image, "orders", None)
                == topic_result(
                    ...
                    vec![
                        ...
                        freeze_entry("frozen:literal:orders", super::CONFIG_SOURCE_DYNAMIC_TOPIC,),
                    ],
                )
        );
    }
    ...
    fn topic_describe_applies_the_key_filter_to_the_synthesised_key() {
        ...
        let retention = super::make_entry(
            config_keys::RETENTION_MS,
            "60000",
            super::CONFIG_SOURCE_DYNAMIC_TOPIC,
        );
        let frozen = freeze_entry("frozen:literal:orders", super::CONFIG_SOURCE_DYNAMIC_TOPIC);
