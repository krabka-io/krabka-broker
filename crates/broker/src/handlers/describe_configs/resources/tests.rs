...
fn a_broker_that_overrides_nothing_still_reports_its_static_configuration() {
    ...
    // the node's own configuration, read-only, and the two KIP-98 expiry
    // keys at `DEFAULT_CONFIG` because this node never moved them off
    // Kafka's built-in defaults.
    let result = describe(
