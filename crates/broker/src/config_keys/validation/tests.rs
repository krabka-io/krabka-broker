...
#[test]
fn validate_diskless_accepts_bools_and_rejects_anything_else() {
    let cases = [
        ("true", true),
        ("false", true),
        ("TRUE", false),
...
fn diskless_and_tiered_storage_exclude_each_other() {
    ...
    for (diskless, tiered, want_ok) in cases {
        let mut overrides = BTreeMap::new();
        if let Some(diskless) = diskless {
            overrides.insert(DISKLESS.to_string(), diskless.to_string());
        }
        if let Some(tiered) = tiered {
            overrides.insert(REMOTE_STORAGE_ENABLE.to_string(), tiered.to_string());
        }
        assert!(
            validate_config_combination(&overrides).is_ok() == want_ok,
            "overrides {overrides:?}"
...
#[test]
fn diskless_plus_tiered_rejection_names_both_keys() {
    let overrides = maplit::btreemap! {
    DISKLESS.to_string() => "true".to_string(),
    REMOTE_STORAGE_ENABLE.to_string() => "true".to_string()};

    let error = validate_config_combination(&overrides).unwrap_err();

    assert!(error.contains(DISKLESS), "got: {error}");
    assert!(error.contains(REMOTE_STORAGE_ENABLE), "got: {error}");
}
