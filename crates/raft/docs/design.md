...
- [`tests/kraft_model.rs`](../tests/kraft_model.rs) is the Stateright model. It holds the real `QuorumStateMachine` for each node, an unordered network with loss and duplication, and node crashes, and it checks election safety over three voters and committed-log linearizability over two.
