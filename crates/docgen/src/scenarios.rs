//! Render Krabka's own deterministic consensus simulator into a Mermaid
//! sequence-diagram "slideshow" markdown page.
//!
//! The diagrams here are not hand-drawn. [`failure_scenarios_md`] runs
//! [`krabka_raft::scenarios::scenarios`] in process and turns each recorded
...
use krabka_raft::scenarios::{ScenarioTrace, TraceAction};

/// The intro paragraph that opens the page.
const INTRO: &str = "\
These diagrams are **generated**, not drawn. Each one is produced by running \
Krabka's own deterministic `KRaft` consensus simulator — the same pure, \
...
pub fn failure_scenarios_md() -> String {
    ...
    out.push_str(INTRO);
    out.push('\n');
    for trace in krabka_raft::scenarios::scenarios() {
...
fn render_scenario(out: &mut String, trace: &ScenarioTrace) {
    ...
    if trace.id == "split_brain_prevented" {
        ...
        let _ = writeln!(
            out,
            "**✓ With Krabka's quorum + pre-vote** — the generated trace from the simulator:\n"
        );
    }
...
Krabka prevents this: a candidate must win a **majority** of votes before it \
