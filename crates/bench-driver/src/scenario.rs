...
//!
//! Every size, extent, and rate here is a [`krabka_units`] quantity, and each
...
mod bounded {
    ...
    bounded_module!(
        ...
        positive_size,
        ByteSize,
        krabka_units::serde_units::human::byte_size::serialize,
        krabka_units::serde_units::human::byte_size::deserialize,
    ...
    bounded_module!(
        ...
        positive_time,
        Time,
        krabka_units::serde_units::human::time::serialize,
        krabka_units::serde_units::human::time::deserialize,
    ...
    bounded_module!(
        ...
        positive_rate,
        Frequency,
        krabka_units::serde_units::human::frequency::serialize,
        krabka_units::serde_units::human::frequency::deserialize,
...
impl Stack {
    ...
    pub fn broker_pod_regex(self) -> &'static str {
        match self {
            // Krabka StatefulSets are `<kafka>-<nodepool>` per the operator.
...
impl Acks {
    pub(crate) fn into_producer(self) -> krabka_client_producer::Acks {
        match self {
            Acks::None => krabka_client_producer::Acks::Zero,
            Acks::Leader => krabka_client_producer::Acks::One,
            Acks::All => krabka_client_producer::Acks::All,
        }
    }
}
...
impl Compression {
    pub(crate) fn into_producer(self) -> krabka_client_producer::Compression {
        match self {
            Compression::None => krabka_client_producer::Compression::None,
            Compression::Gzip => krabka_client_producer::Compression::Gzip,
            Compression::Snappy => krabka_client_producer::Compression::Snappy,
            Compression::Lz4 => krabka_client_producer::Compression::Lz4,
            Compression::Zstd => krabka_client_producer::Compression::Zstd,
        }
    }
}
...
mod tests {
    ...
    fn stack_broker_pod_regex_distinguishes_stacks() {
        assert2::assert!(Stack::Krabka.broker_pod_regex() == "^demo-broker");
        assert2::assert!(Stack::Kafka.broker_pod_regex() == "^demo-kafka-");
        // The krabka prefix must match BOTH the single-pool e2e naming and the
        // multi-pool bench naming (used by failover.rs `starts_with`).
        let p = Stack::Krabka.broker_pod_regex().trim_start_matches('^');
    ...
    fn acks_map_to_producer_enum() {
        use krabka_client_producer::Acks as P;
        for (acks, want) in [
            (Acks::None, P::Zero),
            (Acks::Leader, P::One),
            (Acks::All, P::All),
    ...
    fn compression_maps_to_producer_enum() {
        use krabka_client_producer::Compression as PC;
